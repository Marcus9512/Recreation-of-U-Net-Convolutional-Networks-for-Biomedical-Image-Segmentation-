...
def augment_and_crop(reps=5):
    ...
    '''
    os.rmdir('data/label') # FIY these are gitignored
    os.rmdir('data/train') # FIY these are gitignored
    os.mkdir('data/label')
    os.mkdir('data/train')
    n = 30
    img_len = 512
    ...
    for i in range(n):
        ...
        for (k, (u, v)) in enumerate(((0,0), (0,256), (256,0), (256, 256))):
            ...
            imageio.imsave('data/train/' + str(4*i+k) + ".jpg", x[u:u+256, v:v+256].clip(0, 256).astype('uint8'))
            imageio.imsave('data/label/' + str(4*i+k) + ".jpg", y[u:u+256, v:v+256].clip(0, 256).astype('uint8'))
        for j in range(reps):
            ...
            x_ij = x_ij.clip(0, 256).astype('uint8')
            y_ij = y_ij.clip(0, 256).astype('uint8')
            idx = 30*4 + i * reps + j
