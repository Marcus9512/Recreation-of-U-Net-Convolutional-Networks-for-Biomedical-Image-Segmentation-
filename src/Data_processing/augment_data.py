...
def augment_and_crop(reps=5):
    ...
    for i in range(n):
        ...
        for (k, (u, v)) in enumerate(((0,0), (0,256), (256,0), (256, 256))):
            ...
            imageio.imsave('data/train/' + str(4*i+k) + ".jpg", x[u:u+255, v:v+255].clip(0, 255).astype('uint8'))
            imageio.imsave('data/label/' + str(4*i+k) + ".jpg", y[u:u+255, v:v+255].clip(0, 255).astype('uint8'))
        for j in range(reps):
